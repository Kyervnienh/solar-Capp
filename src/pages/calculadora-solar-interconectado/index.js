        ...
        <>
            ...
            {panel === panels[2] ?
                <div className="solarCalcContainer" >
                    ...
                    <SelectSolarPanel solarPanel={solarPanel} solarPanelsInfo={solarPanelsInfo} setSolarPanel={setSolarPanel} />
                    ...
                    <div className="energySavingCont">
                        <div className="energySavingSection" >
                            ...
                            <input className="solarCalcInputElement interInput1" type="tel" pattern="[0-9]*"
                                value={solarPanelQuantity ? solarPanelQuantity : ""}
                                onChange={e => setSolarPanelQuantity(e.target.validity.valid ? e.target.value : solarPanelQuantity)
                                } />
                        </div>
                        <div className="energySavingSection">
                            <p className="solarCalcTxtElement">Consumo eléctrico mensual (kWh):</p>
                            <input className="solarCalcInputElement interInput" type="tel" pattern="[0-9]+([\.,][0-9]{0,2})?"
                                value={consum ? consum : ""} onChange={e => {
                        ...
                        <div className="energySavingSection">
                            ...
                            <input className="solarCalcInputElement interInput" type="tel" pattern="[0-9]+([\.,][0-9]{0,2})?"
                                value={cost ? cost : ""} onChange={e => setCost(e.target.validity.valid ? e.target.value : cost)} />
                        </div>
                        <p className="solarCalcTxtElement energySavingTxt">Consumo eléctrico mensual esperado al implementar el sistema fotovoltaico:
                            <span className="solarCalcTxtElementSpan interSpan">{Math.round((1 - (solarPanelQuantity * solarPanel[1].power / minPower)) * consum || 0) + " kWh"}</span></p>
                        <p className="solarCalcTxtElement energySavingTxt">Costo del consumo eléctrico mensual esperado:
                            <span className="solarCalcTxtElementSpan interSpan">{"$ " + Math.round((1 - (solarPanelQuantity * solarPanel[1].power / minPower)) * cost || 0)}</span></p>
                        <p className="solarCalcTxtElement energySavingTxt">Porcentaje de ahorro:
                            <span className="solarCalcTxtElementSpan interSpan">{Math.round(100 * (solarPanelQuantity * solarPanel[1].power / minPower) || 0) + " %"}</span></p>
                        <div className="energySavingSection">
                            <p className="solarCalcTxtElement">Costo total de los paneles solares ($):</p>
                            <input className="solarCalcInputElement interInput" type="tel" pattern="[0-9]+([\.,][0-9]{0,2})?"
                                value={panelCost ? panelCost : ""} onChange={e => setPanelCost(e.target.validity.valid ? e.target.value : panelCost)} />
                        </div>
                        <p className="solarCalcTxtElement energySavingTxt">Tiempo de retorno de inversión:
                            <span className="solarCalcTxtElementSpan interSpan">{(panelCost / (12 * cost * solarPanelQuantity * solarPanel[1].power / minPower) || 0).toFixed(1) + " años"}</span></p>
